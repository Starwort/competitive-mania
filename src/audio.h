 ...
 * @param loop          0 ends after playing once (sound), 1 repeats and fades when
 * other music added (music)
 * @param volume        Volume, read playSound()
 *
 * @return returns a new Audio or NULL on failure, you must call freeAudio() on return
 * Audio
 *
 */
 ...
 * Note, this needs to be run even if initAudio fails, because it frees the global audio
 * device
 *
 */
void endAudio(void);
