...
/* The rate at which the volume fades when musics transition. The higher number
 * indicates music fading faster */
#define AUDIO_MUSIC_FADE_VALUE 2

/* Flags OR'd together, which specify how SDL should behave when a device cannot offer a
 * specific feature If flag is set, SDL will change the format in the actual audio file
 * structure (as opposed to gDevice->want)
 *
 * Note: If you're having issues with Emscripten / EMCC play around with these flags
 *
 * 0                                    Allow no changes
 * SDL_AUDIO_ALLOW_FREQUENCY_CHANGE     Allow frequency changes (e.g. AUDIO_FREQUENCY is
 * 48k, but allow files to play at 44.1k SDL_AUDIO_ALLOW_FORMAT_CHANGE        Allow
 * Format change (e.g. AUDIO_FORMAT may be S32LSB, but allow wave files of S16LSB to
 * play) SDL_AUDIO_ALLOW_CHANNELS_CHANGE      Allow any number of channels (e.g.
 * AUDIO_CHANNELS being 2, allow actual 1) SDL_AUDIO_ALLOW_ANY_CHANGE           Allow
 * all changes above
 */
// #define SDL_AUDIO_ALLOW_CHANGES SDL_AUDIO_ALLOW_ANY_CHANGE
#define SDL_AUDIO_ALLOW_CHANGES \
 ...
 * @param audio         Provide an Audio object if copying from memory, or NULL if using
 * a filename
 ...
 */
static inline void
    playAudio(const char* filename, Audio* audio, uint8_t loop, int volume);
...
void initAudio(void) {
    ...
    if (gDevice == NULL) {
        fprintf(
            stderr,
            "[%s: %d]Fatal Error: Memory c-allocation error\n",
            __FILE__,
            __LINE__);
        return;
    }
    ...
    if (!(SDL_WasInit(SDL_INIT_AUDIO) & SDL_INIT_AUDIO)) {
        fprintf(
            stderr,
            "[%s: %d]Error: SDL_INIT_AUDIO not initialized\n",
            __FILE__,
            __LINE__);
        return;
    }
    ...
    if ((gDevice->device = SDL_OpenAudioDevice(
             NULL, 0, &(gDevice->want), NULL, SDL_AUDIO_ALLOW_CHANGES))
        == 0) {
        fprintf(
            stderr,
            "[%s: %d]Warning: failed to open audio device: %s\n",
            __FILE__,
            __LINE__,
            SDL_GetError());
    } else {
...
static inline void
    ...
    if (filename != NULL) {
        ...
        if (newAudio == NULL) {
            fprintf(
                stderr,
                "[%s: %d]Fatal Error: Memory allocation error\n",
                __FILE__,
                __LINE__);
            return;
        }
        ...
        fprintf(
            stderr,
            "[%s: %d]Warning: filename and Audio parameters NULL\n",
            __FILE__,
            __LINE__);
        return;
    }
...
static inline void audioCallback(void* userdata, uint8_t* stream, int len) {
    ...
    while (audio != NULL) {
        if (audio->length > 0) {
            ...
            SDL_MixAudioFormat(
                stream, audio->buffer, AUDIO_FORMAT, tempLength, audio->volume);
